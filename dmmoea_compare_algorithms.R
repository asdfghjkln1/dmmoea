...
compare_algorithms <- function(){
  ...
  results.path <- args[2] #"Tests\\runs\\XieBeni"
  ...
  for(j in 1:length(datasets)){
    ...
    #data.diversity <- plot.data.diveristy[plot.data.diversity$Dataset == dataset, ]+
    #block <- as.numeric(factor(data$Dataset))
    #data[, "block"] <- block
...
friedman.test.with.post.hoc <- function(formu, data, to.print.friedman = T, to.post.hoc.if.signif = T,  to.plot.parallel = T, to.plot.boxplot = T, signif.P = .05, color.blocks.in.cor.plot = T, jitter.Y.in.cor.plot =F, dataset)
{
  ...
  {
    if(pvalue(the.sym.test) < signif.P || TRUE)
    {
      # the post hoc test
      ...
      if(to.plot.boxplot)
      {
        ...
        #print(Y.b.minus.a.combos)
        #plot <- ggplot(Y.b.minus.a.combos, aes(x=Dataset, y=Hypervolume, fill=Dataset, color=is.signif.color)) +
        #  labs(title=paste(Y.name," difference distribution"), tag=paste0("Dataset ", dataset)) +
        #  geom_boxplot() +
        ...
        boxplot(Y.b.minus.a.combos,
                ...
                col = is.signif.color,
                main = paste(Y.name," difference distribution"),
                ylim = the.ylim,
