...
diverse_memetic_nsga2 <- function(distances, params, output.path, initial_population=NULL, limits=NULL, debug=FALSE, plot=FALSE, calculate.diversity=FALSE, experiment.name="dmnsga2", base.path=NULL){
  ...
  if(plot){
    plot_phase_population(P, 0, output.path, limits, show.all=TRUE)
  }
  generations.no.changes <- 0
  has.converged <- FALSE
...
plot_pareto <- function(old.pareto, new.pareto, generation, output.path, limits, agent=NULL, phase=NULL){
  ...
  new.pareto <- normalise_pareto(new.pareto[, c("f1", "f2")], limits)
  old.pareto <- normalise_pareto(old.pareto[, c("f1", "f2")], limits)
...
plot_phase_population <- function(solutions, phase, output.path, limits, is_final=FALSE, show.all=FALSE){
  output.path <- file.path(output.path, "pareto")
  ...
  population <- normalise_pareto(solutions[ , c("f1", "f2")], limits)
...
plot_algorithm_comparison_pareto <- function(exp.path, load.data = FALSE, limit.run=Inf){
  ...
  if(!load.data){
    ...
    selected <- read.table(file.path(exp.path, "data_pareto.csv"), sep=",", header=TRUE, row.names=NULL)
    print("Loaded pareto")
    print(selected)
    ...
    for(i in 1:length(algorithms)){
      ...
      for(j in 1:length(datasets)){
        pareto.dataset <- as.data.frame(matrix(nrow=0, ncol=2)) 
        dataset <- datasets[j]
  ...
  for(i in 1:length(datasets)){
    ...
    print("Plotting data from levels:")
  ...
  for(i in 1:length(algorithms)){
    ...
    for(j in 1:length(datasets)){
      ...
      for(k in 1:length(experiments)){
        ...
        if(!file.exists(file.path(exp.name, "population.csv"))){  next  }
        #pareto[[f]] <- read.csv(file.path(experiments[f], paste0(exp.name, ".csv")), header = FALSE)
        pareto <- read.table(file.path(exp.name, paste0(basename(exp.name), ".csv")), sep=",", header = FALSE, row.names=NULL)
        max.values <- apply(pareto, 2, max)
